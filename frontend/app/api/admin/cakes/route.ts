...
type NewCake = { name: string; slug: string; description?: string; ingredients?: string; allergens?: string; basePrice: number; categoryIds: string[]; variants: NewVariant[] };
...
function parseNewCake(value: unknown): NewCake | null {
  ...
  return { name: name.trim(), slug, description: isText(description, 4000) ? description.trim() : undefined, ingredients: isText(ingredients, 4000) ? ingredients.trim() : undefined, allergens: isText(allergens, 2000) ? allergens.trim() : undefined, basePrice, categoryIds, variants };
}
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const cake = await getPrismaClient().cake.create({ data: { name: input.name, slug: input.slug, shortDescription: input.description, description: input.description, ingredients: input.ingredients, allergens: input.allergens, basePrice: input.basePrice, categories: { create: input.categoryIds.map((categoryId) => ({ categoryId })) }, variants: { create: input.variants.map((variant, index) => ({ ...variant, isDefault: index === 0 })) } }, include: { variants: true } });
