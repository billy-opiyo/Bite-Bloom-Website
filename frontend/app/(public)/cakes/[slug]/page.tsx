...
export default function CakeDetailPage() {
  ...
  useEffect(() => {
    ...
    async function loadCake() {
      try {
        ...
        if (!controller.signal.aborted) setError(cause instanceof Error ? cause.message : "Cake not found.");
      } finally {
  ...
  return <main className="product-page"><header className="product-header"><div className="container"><Link className="text-link" href="/cakes">← All cakes</Link></div></header><section className="container product-layout" aria-labelledby="product-title"><div className="product-visual catalog-cake-placeholder"><span aria-hidden="true">✦</span><small>Product imagery will appear here once media is attached.</small></div><div className="product-copy"><p className="eyebrow">{cake.categories[0]?.name || "Cake"}</p><h1 id="product-title">{cake.name}</h1><p className="product-lede">{cake.fullDescription || cake.description || "Made fresh for your celebration."}</p><strong className="product-route-price">From {money(selectedPrice, cake.currency)}</strong>{cake.preparationTime && <p className="product-prep">Prepared with at least {cake.preparationTime} hours&apos; notice.</p>}<div className="product-options"><div className="product-option"><strong>Choose a size</strong><div className="product-option-grid">{cake.variants.map((variant) => <button className={selectedVariant?.id === variant.id ? "selected" : ""} key={variant.id} onClick={() => setSelection((current) => ({ ...current, variantId: variant.id }))} type="button">{variant.name}<small>{money(variant.price, cake.currency)}</small></button>)}</div></div>{cake.customizations.map((item) => <div className="product-option" key={item.key}><strong>{item.label}{item.isRequired ? " · Required" : ""}</strong>{item.type === "TEXT" ? <textarea value={String(selection[item.key] || "")} onChange={(event) => setSelection((current) => ({ ...current, [item.key]: event.target.value }))} placeholder={`Add ${item.label.toLowerCase()}`} /> : <div className="product-option-grid">{item.values.map((value) => <button className={selection[item.key] === value.value ? "selected" : ""} key={value.value} onClick={() => setSelection((current) => ({ ...current, [item.key]: value.value }))} type="button">{value.label}{value.priceDelta ? <small>+ {money(value.priceDelta, cake.currency)}</small> : null}</button>)}</div>}</div>)}</div>{message && <p className="product-message" role="status">{message}</p>}<button className="button button-dark product-add-button" onClick={() => void addToCart()} type="button">Add to cart · {money(selectedPrice, cake.currency)}</button><p className="product-note">Prices and customization validity are confirmed again by the server when added to cart.</p></div></section><PublicFloatingActions message={`Hi Bite & Bloom, I have a question about ${cake.name}.`} /></main>;
}
