...
export default function CakeDetailPage() {
  ...
  const images = cake?.images.filter((image): image is typeof image & { url: string } => Boolean(image.url)) || [];
  const visibleImages = images.filter((image) => !failedImages[image.url]);

  useEffect(() => {
    if (activeImage >= visibleImages.length && visibleImages.length > 0) setActiveImage(0);
