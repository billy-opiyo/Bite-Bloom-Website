  ...
  return (
    <div className="site-shell">
      ...
      <main id="top">
        ...
        {order && <section className="order-tracking-section" id="order-tracking"><div className="container"><div className="tracking-header"><div><p className="eyebrow">Your cake is on its way</p><h2>Order <em>{order.number}</em></h2><p>Expected {order.dateLabel} · {order.method === "delivery" ? "Home delivery" : "Pickup from Kilimani studio"}</p></div><a className="whatsapp-button" href={whatsappLink(whatsappMessage)} target="_blank" rel="noreferrer"><FaWhatsapp aria-hidden="true" /> WhatsApp support</a></div><div className="tracking-line">{orderStatuses.map((status, index) => <div className={`tracking-step ${index <= order.statusIndex ? "complete" : ""} ${index === order.statusIndex ? "current" : ""}`} key={status.label}><span className="tracking-icon"><Icon name={status.icon} size={17} /></span><div><strong>{status.label}</strong><p>{status.note}</p></div></div>)}</div><div className="tracking-footer"><span><Icon name="clock" size={16} /> We&apos;ll send updates by SMS and WhatsApp</span><button onClick={() => setOrder((current) => current ? { ...current, statusIndex: Math.min(current.statusIndex + 1, orderStatuses.length - 1) } : current)}>Preview next update <Icon name="arrow" size={14} /></button></div></div></section>}
