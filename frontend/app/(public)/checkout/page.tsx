...
export default function CheckoutPage() {
  ...
  useEffect(() => {
    ...
    void fetch(`/api/checkout/slots?date=${encodeURIComponent(scheduledDate)}`).then(async (response) => {
      ...
      if (cancelled) return;
      if (!response.ok || !payload?.data?.slots) { setSlotMessage(payload?.error?.message || "Live slot availability is unavailable. Try again shortly."); return; }
      const nextSlots = payload.data.slots;
      setSlots(nextSlots);
      setDeliverySlot((current) => nextSlots.find((slot) => slot.slot === current && slot.available)?.slot || nextSlots.find((slot) => slot.available)?.slot || "");
      setSlotMessage(nextSlots.some((slot) => slot.available) ? "Choose an available slot." : "This date is full. Choose another date.");
    }).catch(() => { if (!cancelled) setSlotMessage("Live slot availability is unavailable. Try again shortly."); });
