...
Baseline security headers/CSP (with development-only `unsafe-eval`), a database-aware sitemap, a tested in-process rate-limit core, a same-origin browser mutation guard, a 1 MiB declared API body-size guard, and middleware JSON content-type validation for non-Auth API mutations are now present; duplicate header configuration has been consolidated. Auth.js form-encoded callbacks remain explicitly exempt from the JSON guard. The rate-limit test also prompted the missing `server-only` dependency to be added explicitly. These guards are defense-in-depth and do not replace distributed limits, WAF/bot protection, or provider-level controls. The npm install reported eight audit findings; dependency upgrades require a separate review.
